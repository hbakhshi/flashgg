...
lepton_variables=[
    ...
    #"ele1_RelIsoEA:= ?(electrons.size>0)? ( electrons.at(0).pfIsolationVariables().sumChargedHadronPt + max(0.0, electrons.at(0).pfIsolationVariables().sumNeutralHadronEt + electrons.at(0).pfIsolationVariables().sumPhotonEt - getElecAlpha(0)*globalVariables.rho) ) / electrons.at(0).pt : -999",
    #"ele2_RelIsoEA:= ?(electrons.size>1)? ( electrons.at(1).pfIsolationVariables().sumChargedHadronPt + max(0.0, electrons.at(1).pfIsolationVariables().sumNeutralHadronEt + electrons.at(1).pfIsolationVariables().sumPhotonEt - getElecAlpha(1)*globalVariables.rho) ) / electrons.at(1).pt : -999",
    ...
    "muon1_chi2             := ?(muons.size>0)? muons.at(0).globalTrack().normalizedChi2 : -999",
    "muon2_chi2             := ?(muons.size>1)? muons.at(1).globalTrack().normalizedChi2 : -999",
    "muon1_mHits            := ?(muons.size>0)? muons.at(0).globalTrack().hitPattern().numberOfValidMuonHits : -999",
    "muon2_mHits            := ?(muons.size>1)? muons.at(1).globalTrack().hitPattern().numberOfValidMuonHits : -999",
    ...
    #"muon1_dxy            := ?(muons.size>0)? muons.at(0).muonBestTrack().dxy(diPhoton().vtx.position) : -999"
    #"muon2_dxy            := ?(muons.size>1)? muons.at(1).muonBestTrack().dxy() : -999"
...
jet_variables=[
    ...
    "n_fwdjets              := nonbJets.size",
    "fwdjet1_pt             := ?nonbJets.size>0? nonbJets.at(0).pt : -999",
    "fwdjet2_pt             := ?nonbJets.size>1? nonbJets.at(1).pt : -999",
    "fwdjet1_eta            := ?nonbJets.size>0? nonbJets.at(0).eta: -999.",
    "fwdjet2_eta            := ?nonbJets.size>1? nonbJets.at(1).eta:-999.",
    "fwdjet1_phi            := ?nonbJets.size>0? nonbJets.at(0).phi: -999.",
    "fwdjet2_phi            := ?nonbJets.size>1? nonbJets.at(1).phi: -999.",
