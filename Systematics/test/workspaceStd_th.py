...
from flashgg.MetaData.samples_utils import *


name,xsec,totEvents_init,files,maxEvents,specialPrepend = customize.samplesMan.getDatasetMetaData(customize.maxEvents , customize.dataset[0] , jobId=-1 , nJobs=customize.nJobs , weightName=customize.WeightName)
print customize.WeightName, totEvents_init
dumper_index = process.p.index(process.tagsDumper)

VarsToAdd = ["dipho_eta              := diPhoton().eta"]

VarsForAllCategories = [ "top_pt := ?top.isNonnull()? GetTop().pt() : -10.0",
                         "higgs_eta := ?higgs.isNonnull()? GetHiggs().eta() : -10.0",
                         "fwdJet_eta := ?fwdJet.isNonnull()? GetFwdJet_().eta() : -10.0",
                         "topDecayMode := topDecayMode"]
for category in process.tagsDumper.categories:
    lst = VarsForAllCategories
    if not category.className == "NoTag":
        lst += VarsToAdd
    vars_ = category.variables
    cfgTools.addVariables( vars_ , lst )

TH_Weights = {'mg_reweight_1':(-3.00,1.00),
              # 'mg_reweight_2':(-2.00,1.00),
              # 'mg_reweight_3':(-1.50,1.00),
              # 'mg_reweight_4':(-1.25,1.00),
              # 'mg_reweight_5':(-0.75,1.00),
              # 'mg_reweight_6':(-0.50,1.00),
              # 'mg_reweight_7':(-0.25,1.00),
              # 'mg_reweight_8':(0.00,1.00),
              # 'mg_reweight_9':(0.25,1.00),
              # 'mg_reweight_10':(0.50,1.00),
              # 'mg_reweight_11':(0.75,1.00),
              'mg_reweight_12':(1.00,1.00),
              # 'mg_reweight_13':(1.25,1.00),
              # 'mg_reweight_14':(1.50,1.00),
              # 'mg_reweight_15':(2.00,1.00),
              # 'mg_reweight_16':(3.00,1.00),
              # 'mg_reweight_17':(-3.00,1.50),
              # 'mg_reweight_18':(-2.00,1.50),
              # 'mg_reweight_19':(-1.50,1.50),
              # 'mg_reweight_20':(-1.25,1.50),
              # 'mg_reweight_21':(-1.00,1.50),
              # 'mg_reweight_22':(-0.75,1.50),
              # 'mg_reweight_23':(-0.50,1.50),
              # 'mg_reweight_24':(-0.25,1.50),
              # 'mg_reweight_25':(0.00,1.50),
              # 'mg_reweight_26':(0.25,1.50),
              'mg_reweight_27':(0.50,1.50),
              # 'mg_reweight_28':(0.75,1.50),
              # 'mg_reweight_29':(1.00,1.50),
              # 'mg_reweight_30':(1.25,1.50),
              # 'mg_reweight_31':(1.50,1.50),
              # 'mg_reweight_32':(2.00,1.50),
              # 'mg_reweight_33':(3.00,1.50),
              # 'mg_reweight_34':(-3.00,0.50),
              # 'mg_reweight_35':(-2.00,0.50),
              # 'mg_reweight_36':(-1.50,0.50),
              # 'mg_reweight_37':(-1.25,0.50),
              # 'mg_reweight_38':(-1.00,0.50),
              # 'mg_reweight_39':(-0.75,0.50),
              # 'mg_reweight_40':(-0.50,0.50),
              # 'mg_reweight_41':(-0.25,0.50),
              # 'mg_reweight_42':(0.00,0.50),
              # 'mg_reweight_43':(0.25,0.50),
              # 'mg_reweight_44':(0.50,0.50),
              # 'mg_reweight_45':(0.75,0.50),
              # 'mg_reweight_46':(1.00,0.50),
              # 'mg_reweight_47':(1.25,0.50),
              # 'mg_reweight_48':(1.50,0.50),
              # 'mg_reweight_49':(2.00,0.50),
              # 'mg_reweight_50':(3.00,0.50),
              # 'mg_reweight_51':(-0.9,),
              # 'mg_reweight_52':(-0.8,),
              # 'mg_reweight_53':(-0.7,),
              # 'mg_reweight_54':(-0.6,),
...
for weight_ in sorted(TH_Weights.keys()) :
    #print weight_ , TH_Weights[weight_]
    if len( TH_Weights[weight_] ) == 2 :
        name = "ct%.2f_cv%.2f" % TH_Weights[weight_]
    else :
        name = "cp%.2f" % TH_Weights[weight_]
    name = name.replace( "-" , "_" )
    name = name.replace( "." , "" )
    setattr( process , name , process.tagsDumper.clone() )
    new_dumper = getattr( process , name ) #"dumper%s" % weight_ )
    new_dumper.LHEWeightName = weight_
    name,xsec,totEvents,files,maxEvents,specialPrepend = customize.samplesMan.getDatasetMetaData(customize.maxEvents , customize.dataset[0] , jobId=-1 , nJobs=customize.nJobs , weightName=weight_)
    #print type(new_dumper.lumiWeight), type(totEvents_init) , type(totEvents) 
    new_dumper.lumiWeight = new_dumper.lumiWeight.value()*totEvents_init/totEvents
    dumper_index += 1
    process.p.insert( dumper_index , new_dumper )


del process.tagsDumper.LHEWeightName
name,xsec,totEvents,files,maxEvents,specialPrepend = customize.samplesMan.getDatasetMetaData(customize.maxEvents , customize.dataset[0] , jobId=-1 , nJobs=customize.nJobs)
process.tagsDumper.lumiWeight = process.tagsDumper.lumiWeight.value()*totEvents_init/totEvents
process.ct_100_cv100 = process.tagsDumper.clone()
dumper_index += 1
process.p.insert( dumper_index , process.ct_100_cv100 )

del process.tagsDumper
