...
version="1"
fggRunJobs.py --load data_jobs.json -d $outdir/data_jobs_${version} -x cmsRun workspaceStd.py maxEvents=-1 -n 500 -q ${queue} -D -P useAAA=1 --no-use-tarball lumiMask=${LM}